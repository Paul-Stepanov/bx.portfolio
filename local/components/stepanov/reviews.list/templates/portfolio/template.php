       ...
       <div class="component">
          <div id="test" class="component__container">
             ...
             <div class="component__form-wrapper">
                <form class="component__form-body" action="<?= POST_FORM_ACTION_URI ?>" method="post">
					<?= bitrix_sessid_post() ?>
                   <label class="component__label-name" for="name">
                      Имя<input class="component__input-name" type="text" name="name" id="name" required
                                placeholder="Введите имя*">
                   </label>
                   <div class="component__wrap-rating">
					   <?php foreach($arResult["RATING"] as $item): ?>
                          <input class="component__input-rating" type="radio" name="rating"
                                 id="rating<?= $item["ID"] ?>" value="<?= $item["ID"] ?>">
                          <label class="component__label-rating" for="rating<?= $item["ID"] ?>"></label>
					   <?php endforeach; ?>
                   </div>

                   <label class="component__label-comment" for="comment">
                      Комментарий<textarea class="component__text-comment" name="comment" id="comment" required
                                           placeholder="Введите текст комментария*"></textarea>
                   </label>

                   <button class="component__submit-button" type="submit">Оставить отзыв</button>
                </form>
             </div>
          </div>
       </div>
	<?php endif; ?>
<script defer='defer' src="<?= $templateFolder ?>/js/jquery/jquery-3.7.0.min.js"></script>
